...
export const getResponsiveOverlayTypography = ({
  ...
  const progressLabelFontSize = Math.max(
    ratioDimension(resolvedHeight, 0.0154 + portraitStrength * 0.0004, 10, 0.25),
    ultraWideStrength > 0 ? 14 : 10
  );
  const progressHeight = atLeast(
    ratioDimension(resolvedHeight, 0.027 + portraitStrength * 0.001, 20, 0.25),
    round(progressLabelFontSize * 1.7)
  );
  const chapterTitleFontSize = ratioDimension(resolvedHeight, 0.036 - portraitStrength * 0.002, 24, 0.25);
  const chapterMetaFontSize = ratioDimension(resolvedHeight, 0.0165 - portraitStrength * 0.001, 12, 0.25);
  const chapterTop = ratioDimension(resolvedHeight, 0.035, 16);
  const progressRadius = atLeast(ratioDimension(progressHeight, 0.18, 5, 0.25), round(progressHeight * 0.16));

  return {
    subtitleFontSize,
    subtitleBottom: ratioDimension(resolvedHeight, 0.056 + portraitStrength * 0.004, 35),
    ...
    chapterGap: ratioDimension(chapterTitleFontSize, 0.15, 5, 0.25),
    chapterCardMinWidth: ratioDimension(resolvedWidth, 0.12 + portraitStrength * 0.035, 128, 0.5),
    chapterCardMaxWidthRatio: clamp(0.58 + portraitStrength * 0.04, 0.58, 0.62),
    chapterCardPaddingX: ratioDimension(chapterTitleFontSize, 0.5, 12, 0.25),
    chapterCardPaddingY: ratioDimension(chapterTitleFontSize, 0.28, 8, 0.25),
    chapterCardRadius: ratioDimension(chapterTitleFontSize, 0.24, 8, 0.25),
    chapterMetaFontSize,
