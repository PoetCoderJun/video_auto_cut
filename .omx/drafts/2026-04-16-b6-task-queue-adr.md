...
**建议结论**：当前 B6 应把队列定义为“持久化、可 lease 回收、可 heartbeat、与 job 状态强耦合的 Test flow 执行层”，而不是通用任务平台；在没有新证据之前，不应改成内存队列或无可靠性语义的简单表。若后续要扩展，再从 durable core 出发拆分 dispatch 与 execution。
