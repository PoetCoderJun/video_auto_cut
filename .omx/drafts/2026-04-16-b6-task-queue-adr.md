...
**建议结论**：当前 B6 决定采用方向 A：把队列定义为“持久化、可 lease 回收、可 heartbeat、与 job 状态强耦合的 Test-only execution layer”，而不是通用任务平台；同时明确它当前是“可恢复但非 fenced”的 durable core。在没有新证据之前，不改成内存队列或无可靠性语义的简单表；若未来出现第 2 个稳定 task type 或需要独立 control plane，再重新评估是否演进到方向 B / C。
