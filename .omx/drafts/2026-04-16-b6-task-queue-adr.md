...
- `claim_next_task()` 采用 `BEGIN IMMEDIATE`，并只做本地 3 次 compare-and-set 式尝试；它刻意不套通用 `retry_turso_operation`，遇到某些瞬时 Turso stream error 会交给 worker loop 外层处理，而不是函数内自动重试。
- 当前 claim 选择的是 `status = QUEUED` 且 `task_id ASC` 的最早任务；这更接近 FIFO-like 顺序，而不是复杂公平调度。
- 需要注意：当前 lease/reclaim **不是 fenced ownership**。任务被 reclaim 之后，旧 worker 如果仍在运行，仍可能继续执行 `run_test()` 并在末尾调用 `set_task_succeeded()` / `set_task_failed()`；这些终态写入没有按 `worker_id` 或 `status = RUNNING` 做 compare-and-set 限制。
...
### 2.5b Enqueue de-dup / reuse

- `enqueue_task()` 不是“每次请求都新建一条任务”。
- 当前同一 `(job_id, task_type)` 如果已经存在 `QUEUED` 或 `RUNNING` 任务，会直接返回已有 `task_id`，不再重复插入。
- 这说明队列当前同时承担了“去重/复用同一工作流任务”的 contract，而不只是 append-only backlog。
...
- 一个重要现实限制是：当前 job status **无法严格区分“已入队未执行”和“正在执行”**。入队后 job 已经是 `TEST_RUNNING`，只是通过 `stage_code = TEST_QUEUED` 暂时表达尚未开跑；真正执行后依然沿用同一主状态。
...
- claim 过程可以提供一次性的 `QUEUED -> RUNNING` ownership 转移，但**不能提供 fenced ownership 保证**；
- job 状态与任务执行结果可以持续落盘。
...
- 明确承认它是“**可恢复但非 fenced**”的执行层：它已经比简单表/内存队列强很多，但还没达到严格所有权防护的程度；
- 方向 B / C 不再作为当前并列决策，只保留为未来触发条件。
...
### 7.2 未来重新评估方向的可判定触发条件

- **转向方向 B**：出现第 2 个稳定 task type，且它具有不同的 payload / handler contract / 失败回边，导致 `services.tasks` 继续承担 dispatch + execution 胶水的成本明显上升。
- **转向方向 C**：长期确认系统只服务单一 Test 工作流，而且团队明确不打算把 queue 演进成多任务平台；此时可以正式把它命名为产品内专用 durable queue，而不是继续保留通用化表象。

## 8. ADR 结论

**结论**：当前 B6 决定采用方向 A：把队列定义为“持久化、可 lease 回收、可 heartbeat、与 job 状态强耦合的 Test-only execution layer”，而不是通用任务平台；同时明确它当前是“可恢复但非 fenced”的 durable core。在没有新证据之前，不改成内存队列或无可靠性语义的简单表；若未来出现第 2 个稳定 task type 或需要独立 control plane，再重新评估是否演进到方向 B / C。
