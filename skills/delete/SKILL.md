...
2. 默认保守：不确定时优先 `KEEP`。
3. 唯一核心原则：只要后一句和前一句属于重复语义，就删除前一句，保留后一句；后一句默认就是纠正后的最终版本。
...
| 前一句和后一句是重复语义 | 前一句 `REMOVE`，后一句 `KEEP` |
...
- 把后一句删掉、却把前一句留着：这是本技能最严重的错误；只要是重复语义，必须删前留后。
