        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(
                ...
                try:
                    ...
                    create_job("job_cleanup_step2", JOB_STATUS_STEP2_CONFIRMED, "user")
                    upsert_job_files(
                    ...
                    upsert_job_files(
                        "job_cleanup_step2",
                        final_video_path=str(Path(tmpdir) / "jobs" / "job_cleanup_step2" / "final.mp4"),
                    )
                    ...
                    Path(tmpdir, "jobs", "job_cleanup_step2", "final.mp4").parent.mkdir(
                        parents=True,
                        exist_ok=True,
                    )
                    Path(tmpdir, "jobs", "job_cleanup_step2", "final.mp4").write_text("x")
                    ...
                    self.assertNotIn("job_cleanup_step2", candidates)
                    self.assertIn("job_cleanup_succeeded", expired)
                    self.assertNotIn("job_cleanup_step2", expired)
                finally:
                    get_settings.cache_clear()
